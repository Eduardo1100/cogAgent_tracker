...
- Stable-threshold measurement tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now also suppress enclosure-side proxy thermometer reads while the target transition is still unresolved. When changing this behavior, keep branch resolution tied to direct target evidence or explicit transition observations, and do not let `use thermometer on <heater>` style proxy reads outrank transition-driving control actions.
...
- Growth tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now reopen room-frontier search after repeated nonproductive local precursor tests when only seed-stage evidence is grounded. When changing this behavior, keep direct precursor-to-locus transfers competitive, but do not let repeated seed focus or container toggles outrank unexplored doors once local growth search has clearly stalled.

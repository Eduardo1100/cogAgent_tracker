...
- [src/agent/env_adapter.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/env_adapter.py): environment adapter layer. `NetHackAdapter` wraps NLE with TTY rendering, human-readable action labels, and automatic dismissal of interactive prompts (`[ynq]`/`[yn]` auto-confirmed with 'y', `--More--` auto-dismissed with space) so the cognitive loop never stalls on unanswerable confirmation dialogs.

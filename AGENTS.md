...
- Search-and-place tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now keep focus progress tied to plausible candidate referents during generic `find a(n) ... thing` tasks. When changing this behavior, avoid letting room/context labels such as `living room` count as candidate focus progress, and keep unfocused object-transport actions behind focus/search actions until a candidate is actually grounded.
...
- Search-and-place tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now retire a grounded candidate after the explicit focus/place semantics are satisfied and one follow-up confirmation still leaves the task incomplete, then reopen search away from destination-container confirmation loops. When changing this behavior, keep retirement tied to observation-backed goal satisfaction, preserve focus-first sequencing on replacement candidates, and avoid benchmark-specific blacklists for decor or fixture objects.
- Search tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now retain episode-local remote-room signals from successful `look in <room>` inspections when those observations contain task-relevant evidence. When changing this behavior, keep remote inspection distinct from actual agent location, and prefer opening or traveling into the signaled room before acting on objects only observed from a distance.
- Candidate and room-detection logic in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) should stay structural and observation-driven. Avoid hard-coding ScienceWorld room names, container modifiers, or benchmark object labels when filtering non-candidate referents; prefer dynamic room/location inference from the current percept and task roles.
...
- State-change search snapshots and shortlist bonuses in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now treat only task-compatible grounded substances as authoritative during search/probe phases. When changing this behavior, do not let unrelated explicit substances such as salts, soap, or ambient air outrank container search or source probing for the target substance.
- State-change search in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now reintroduces room-frontier pressure after repeated nonproductive local probes in a multi-exit room. When changing this behavior, keep local source candidates available, but prefer broadening search across reachable exits before spending more actions on unrelated closed containers or non-target focus actions.
...
- State-change container search in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now retires a container from local search only after an observation-backed probe confirms the interior was actually revealed. When changing this behavior, do not treat closed-container inspections or bare `open <container>` affordance changes as exhausted search evidence unless the resulting observation exposes the container contents.
...
- Measurement `locate_instrument` search now also penalizes bouncing back into already-searched rooms or repeating stale local scans once the local search frontier is exhausted. When changing this behavior, keep unexplored doors and room-frontier moves ahead of revisiting exhausted room referents unless new instrument evidence appears.
- Measurement tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now prioritize activation-style control on a hidden target's enclosing device before proxy thermometer reads or direct hidden-target manipulation while transition evidence is still missing. When changing this behavior, keep the preference tied to enclosure-backed measurement state rather than hard-coded device names or benchmark-specific room affordances.
- Stable-threshold measurement tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now escalate from baseline direct readings into powered thermal-device setup when a direction-compatible control candidate and target-transfer affordance are both available. When changing this behavior, keep the preference tied to direction-aware device affordances plus observation-backed transfer actions, and avoid reopening repeated direct thermometer or weak-heater loops until that powered path has been exhausted.
- Comparative focus tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now retain compared targets separately from the final focus target. When changing this behavior, preserve material-label distinctions such as unknown material C vs H, keep evidence-gathering on the compared targets or their test setup, and only promote the final focus target after observation-backed comparison evidence resolves the winner.
- Comparison search in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) now treats `locate_primary_target` as a real room-search phase before any compared target is grounded. When changing this behavior, prefer inspecting the current room once and then reopening room-frontier pressure across visible doors before manipulating unrelated local containers, relation actions, or object transport.
...
- Some conditional trait-branch tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) are also growth/evidence-creation tasks. When changing this behavior, allow `growth_task` and `conditional_branch_task` to coexist, keep precursor-generation or plant-reveal steps ahead of branch-box actions, and do not let early box-facing actions bypass unresolved evidence generation.
- Conditional branch tasks in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) can now resolve branch destinations from transfer clauses such as `place it in the red box`, not just `focus on ...`. When changing this behavior, keep branch-specific destinations out of fixed `destination_container` bias before evidence resolves, and prefer committing the evidence target to the resolved destination over focusing the destination itself.
- Generic conditional-branch evidence gathering in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) should stay evidence-first. Before a branch resolves, prefer inspect / focus / explicit support-device actions over relocating the evidence target or building arbitrary relation chains that are only justified by touching that target.

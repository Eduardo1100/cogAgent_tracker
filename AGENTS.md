...
- Exploration task hints in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) (`_EXPLORATION_TASK_HINTS`) now include gerund forms ("exploring", "exploring the", "collecting treasure", "navigating the") alongside base forms so that tasks phrased with present participles (e.g. "exploring the dungeon") correctly activate `exploration_task=True`. In exploration tasks, the `relocation` action family is protected from hypothesis-ledger deprioritization — wall bumps should not penalise movement when movement is the core mechanism. When adding new exploration hints, prefer environment-agnostic verb phrases over benchmark-specific keywords.

...
- [src/agent/env_adapter.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/env_adapter.py): environment adapter layer. `NetHackAdapter` wraps NLE with TTY rendering, human-readable action labels, automatic dismissal of interactive prompts (`[ynq]`/`[yn]` auto-confirmed with 'y', `--More--` auto-dismissed with space), and a **spatial surroundings summary** (`_parse_surroundings`) appended to every observation. The surroundings block classifies all eight adjacent cells (wall, floor, corridor, stairs, creature, item, etc.) and lists passable vs. blocked directions, giving the cognitive agents structured directional awareness without requiring them to parse the raw ASCII grid.
...
- Small Python changes:
  ...
  - If you touch the local iteration helpers under [src/automation](/home/eduardo/Projects/cogAgent_tracker/src/automation), [scripts/get_latest_experiment.py](/home/eduardo/Projects/cogAgent_tracker/scripts/get_latest_experiment.py), or [scripts/iterate_agent.py](/home/eduardo/Projects/cogAgent_tracker/scripts/iterate_agent.py), run `uv run pytest tests/test_iteration_workflow.py`.
...
- Thinking_Agent candidate ranking in [src/agent/configs/prompts.yaml](/home/eduardo/Projects/cogAgent_tracker/src/agent/configs/prompts.yaml) now instructs the agent to rank multiple untested candidates within the same action family by semantic plausibility before proposing the next one to test. The guidance distinguishes utility-access surfaces (tables kept for regular use), storage/display receptacles (shelves, category-specific stands), and incidental surfaces (seats). When updating this guidance, keep it abstract — do not name specific furniture or environment objects. The goal is to avoid wasting tests on structurally available but semantically mismatched candidates (see cogfix-13 branch).
...
- Recipe-document shortlist penalty in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py) uses `_read_recipe_doc_labels: set[frozenset[str]]` to track which recipe documents have been consumed this episode. The penalty (-65) is applied in `_score_action_for_shortlist` via `_action_targets_read_recipe_doc`. The magnitude must exceed the combined token-overlap bonus a recipe title receives from matching artifact target tokens (typically 40–70 pts); do not lower below -55 without checking that recipe-document actions stay out of the top-8 shortlist in combine_or_transform phase.
...
- Navigation outcomes are tracked in `_navigation_outcomes: dict[tuple[str, str], str]` (keyed by `(source_room_sig, action)`, value is `dest_room_sig`). Populated by `_update_exploration_search_tracking` whenever a nav action moves the agent to a new room. `_get_known_revisit_exits(current_room)` returns the set of nav actions from the current room whose destination has `visit_count >= 1`; these exits receive a -4 penalty in exploration-task shortlist scoring (`known_revisit_exits` in the scoring context). Do not raise the penalty above 4 — the agent may still need to pass through visited rooms when they are the only gateway deeper into unexplored territory.
- Forced periodic deliberation in [src/agent/gwt_agent.py](/home/eduardo/Projects/cogAgent_tracker/src/agent/gwt_agent.py): Thinking_Agent is now invoked every `_FORCED_DELIBERATION_INTERVAL` (5) actions even when Belief_State_Agent shows no uncertainty keywords. This prevents long stretches of unreflective action cycling (e.g. aimless movement loops where the agent confidently repeats unproductive actions). The counter `_actions_since_last_thinking` increments on each new action and resets when Thinking_Agent is selected. Do not raise the interval above 7 or disable it — the original failure mode (25 wasted actions without strategic correction) was the primary bottleneck in open-world tasks.
